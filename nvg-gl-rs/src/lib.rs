...
// use libc::c_uint;
// use nvg_rs::{renderer::{*, self}, context::{ImageFlags, Vertex, ImageId, Paint, CompositeOperationState, Path, BlendFactor}, math::{Extent, Transform, Bounds}, color::Color};
// use slab::Slab;
// use std::ffi::c_void;
...
//         gles_rs::shader_source(
//             vert,
//             &(vert_source.to_str().unwrap()),
//         );
//         gles_rs::shader_source(
//             frag,
//             &(frag_source.to_str().unwrap()),
//         );
...
//     fn drop(&mut self) {
//         unsafe { gles_rs::delete_texture(self.tex) }
//     }
// }
...
//             let mut vert_arr: gles_rs::ffi::GLuint = std::mem::zeroed();
//             gl::GenVertexArrays(1, &mut vert_arr);

//             let mut vert_buf: gles_rs::ffi::GLuint = std::mem::zeroed();
//             gl::GenBuffers(1, &mut vert_buf);

//             gl::UniformBlockBinding(shader.prog, shader.loc_frag, 0);
//             let mut frag_buf: gles_rs::ffi::GLuint = std::mem::zeroed();
...
//         let tex = unsafe {
//             let mut tex: gles_rs::ffi::GLuint = std::mem::zeroed();
...
//     unsafe {
//         let mut data: [gles_rs::ffi::GLchar; 512 + 1] = std::mem::zeroed();
//         let mut len: gles_rs::ffi::GLsizei = std::mem::zeroed();
